...
def filter_test_control(patch: str) -> str:
    ...
    for line in patch.splitlines(keepends=True):
        if line.startswith("diff --git "):
            if current and not TEST_CONTROL.fullmatch(path):
                kept.extend(current)
            current = [line]
...
class SecureVerifiedMixin:
    ...
    async def stage_verifier(self, runtime: Runtime) -> None:
        ...
        patch = await runtime.read("/tmp/prime-agent.patch")
        filtered = filter_test_control(patch)
        if filtered != patch:
            await runtime.write("/tmp/prime-agent.patch", filtered)
        result = await runtime.run(
