...
| `request-sent` | Payload handed to the provider client (body size in `requestBytes`) | Client-side request build (auth, params, extension payload hooks) is slow |
| `first-byte` | HTTP response headers received | Upload of the request body plus provider TTFB (prefill, prompt-cache miss, queueing) |
...
The final entry (`msg: "request timing summary"`) carries every delta at once: `phases.dispatchToPromptBuiltMs`, `promptBuiltToRequestSentMs`, `requestSentToFirstByteMs`, `firstByteToFirstTokenMs`, `firstTokenToStreamDoneMs`, plus `contextEntries`, `requestBytes`, `sessionId`, and the final `usage`.
