...
/**
 * Sequential kernel starts used to re-pay the full warm-path validation every
 * time: python-skill hashing, a complete runtime source re-hash, and a
 * RUNTIME_READY_CHECK interpreter spawn. A successful validation is now cached
 * for the process lifetime, keyed by env, python skills, and runtime identity,
 * and guarded by the bootstrap marker's stat identity: the next start reuses it
 * until the key changes (skills, venv override, runtime source) or the marker
 * changes (external rebuild or wipe), and any failure drops the entry so the
 * following start revalidates from scratch.
 */
