...
describe("AgentSession rlm recursion", () => {
	...
	it("admits a same-name respawn at the delete receipt while the daemon catalog still lists the child", async () => {
		...
		// cleanup open keeps that window: the catalog still lists the closing child
		// under its name when the respawn runs.
		let releaseCleanup: () => void = () => {};
		const cleanupGate = new Promise<void>((resolve) => {
			releaseCleanup = resolve;
		});
		...
		let parentState: ActiveSessionState | undefined;
		const agentMessageController = internals.createAgentMessageController(() => parentState);
		...
		const root = createSession({
			...
			subagentRuntimeHost: {
				createRlmSubagentRuntime: async (options) => {
					const hosted = createSession({ depth: 1, streamFn: gatedStream });
					hostedChildren.push(hosted);
					internals.sessions.set(`child-active-${hostedChildren.length}`, {
						activeSessionId: `child-active-${hostedChildren.length}`,
						clients: new Set(),
						pendingAttaches: 0,
						lastEventSequence: 0,
						runtime: {
							metadata: {
								...
								...(root.sessionFile ? { parentSessionFile: root.sessionFile } : {}),
							},
							session: hosted,
							cwd: tempDir,
						},
		...
		// The closing child is still resident, so the daemon catalog still answers
		// with its name at depth 1 under this parent.
